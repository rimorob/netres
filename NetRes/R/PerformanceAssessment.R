...
##' @author Fred Gruber
NetRes$set("private", "network_performance", function(true_igraph, edges, Nboot = 200, ci = FALSE, seed = 123,cutoff=0.5,oracle=NULL,oracle_name="Oracle",debug=FALSE) {
  require(ROCR)
  require(precrec)
  require(SID)
  ...
  if(length(unique(trueval))==2){
      ...
      toplotlift=tibble(lift=lift,
                        RPP=RPP,
                        model='Inferred')
      if(!is.null(oracle)){
          predorac=prediction(comb_edges$Oracle,comb_edges$True)
          perforac=performance(predorac,"lift","rpp")
          liftorac=perforac@y.values[[1]]
          rpporac=perforac@x.values[[1]]
          toplotlift=bind_rows(
              toplotlift,
              tibble(
                  lift=liftorac,
                  RPP=rpporac,
          ...
          ggp_lift <- toplotlift %>%
              ggplot(aes(x=RPP, y=lift, colour=model))+
          ...
          if(is.null(oracle)){
              ...
              resampled_scores= list(
                  ...
                  resampled_scores= list(
                      ...
                      modnames=c("Inferred",oracle_name),
                      dsids=1:Nboot
                  ) %>% set_names(c("scores","labels","modnames","dsids"))
      ...
      if(!is.null(oracle)){
          ...
          aucs_orac=auc(sscurves_auc) %>%
              filter(modnames==oracle_name) %>% 
              group_by(curvetypes) %>%
              summarise(AUC = mean(aucs))
          auc_roc_orac=filter(aucs_orac, curvetypes == "ROC")$AUC
          auc_prc_orac=filter(aucs_orac, curvetypes == "PRC")$AUC
      }else{
          aucs <- auc(sscurves_auc) %>%
              group_by(curvetypes) %>%
              summarise(AUC = mean(aucs))
      ...
      if(!is.null(oracle)){
          ggp_prc=ggp_prc+
              ggtitle(sprintf(
                  "PR Curve. AUC = %0.2g.\nRandom: %0.2g\nAUCorac= %0.2g",
                  signif(auc_prc, 2), prc_random,auc_prc_orac
          ))
      }else{
          ggp_prc=ggp_prc+
          ggtitle(sprintf(
      ...
      if(!is.null(oracle)){
          ggp_roc=ggp_roc+ggtitle(sprintf(
              "ROC. AUC = %0.2g.\nRandom: %0.2g. \nAUCorac=%0.2g\nroc.test pvalue =%0.2g",
              signif(auc_roc, 2), 0.5, auc_roc_orac,roctest$p.value
          ))
      }else{
          ggp_roc=ggp_roc+ggtitle(sprintf(
              "ROC. AUC = %0.2g.\nRandom: %0.2g",
