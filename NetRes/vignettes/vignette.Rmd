...
netResObj = NetRes$new(generated$data, true.graph = generated$graph, nIter = 10, nBoot=100, algorithm='tabu', 
                       algorithm.args = tabuArgs, mode='foo', latentSpaceMethod='NNMF') #'sparse.pca') 
...
Example 3 - a tiny example of scale-free network generation and inference
```{r, include=TRUE}
pdf('Example3.pdf')
con <- file("test.log")
sink(con, append=TRUE)
...
generated = network$generateData(numSamples=100, latIdx = c(1, 2, 3), rescaleNoiseBy=NULL)
print(network)
bnlearn::graphviz.plot(generated$graph, layout='dot', highlight = list(nodes = grep('U_', colnames(generated$data), value=T), col = "tomato", fill = "orange"))
