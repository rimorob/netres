...
library(igraph)
library(pcalg)
getEnsemble2 = function(train, boot_orders = NULL, Nboot = 50, algorithm = "hc", parallel = FALSE, output = NULL, ...){
    if(!is.null(output)){
    ...
    pdaboot = foreach(ii = 1:Nboot,
                      .combine = mycomb, .packages = c("bnlearn", "foreach", "igraph","dplyr", 'pcalg'),
                      .export = c('boot_orders')) %op% {
                          if (is.null(boot_orders)) {
                              bootorder = sample(1:nrow(train), nrow(train), replace = TRUE)
                          } else {
                              bootorder = boot_orders[[ii]]
                          }
                          ...
                          edgedf = arc.strength(net,trainb) %>% as.data.frame %>%
                              mutate(Boot = ii)
                          print(paste('Done with network', ii))
...
findLatentVars <- function(resDev, scale. = FALSE, nIter = 100, method = 'kernel', kernel='rbfdot', alpha = 0.05, pval_method = 'empirical',
    ...
    if (method == 'autoencoder') {
        ...
        prVarShuf = foreach (ii = 1:nIter, .combine='rbind') %do% {
            cat(paste(ii, 'iterations out of', nIter, "\r"))
...
latentDiscovery = function(
	...
	message("Run bnlearn")
	    newens = getEnsemble2(
		newdata,
                boot_orders = oldens$boot_orders,
		blacklist = blacklist,
...
getResultsGES = function(allnet, toy.data, remove = NULL){
                          ...
                          map_df(1:npp,
                                 function(ii){
                                   ...
                                   if(!is.null(remove)){
                                     ...
                                     iikeep = setdiff(1:length(allnodes), which(allnodes %in% remove))
                                     est.cpdag = est.cpdag[iikeep, iikeep]
                                   }
                                   compute_metrics(true.dag = toy.data$true.obs.cpdag.amat, 
