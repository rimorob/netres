...
To summarize, to detect latent space $U$, define as $R^*$ the residuals when $U$ is unknown and can't be used in estimation - when we only know $G$.   Now $\exists i, j s.t. R^*_i \not\perp\!\!\!\perp R^*_j | \bar{C}_i, \bar{C}_j$. Thus, it is the violation of independence of residuals given graph that gives rise to identifiability. 
\end{proof}
\end{theorem}
