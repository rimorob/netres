...
Posit that we know the true DAG $G$ over $D$, but not $G_u \setminus G$, the latent space's edges to observed variables.  Assume that $L$ is orthogonal, with no variable in $D$ a parent of any variable in $L$. Define as $R_C = C^{L,G} - \bar{C}^{L,G}$ the residuals of all children of $L$ that also have parents in $G$.  If relationships described by $G$ are linear, we can infer $L$ from $R^*_{L,C}$ up to sign and scale.  

\begin{proof}
Consider $G_u$.  Since all variables in $L$ are upstream of $C^{L,G}$, and since all residuals $R_{L,C}$ are downstream of $C^{L,G}$, we can write $L \perp\!\!\!\perp R_{L,C^{L,G}} | C^{L,G}$.  Therefore, all paths $L \rightarrow R$ have form $L \rightarrow C^{L,G} \rightarrow R_{L,C^{L,G}}$.  Further, $R_i \perp\!\!\!\perp R_j | C_i,C_j,L,   \forall C_i, C_j \in C^{L,G}$. Also, $L_i \perp\!\!\!\perp L_j, \forall i, j \in L$ by definition of orthogonality.  Therefore, no paths exist in $G_u$ within $L$ or within $R$, so it is a bipartite graph with respect to the variables in $L$ and $R$.  \cite{anandkumar_learning_2013} showed that, if such a bipartite graph met the expansion property, the latent space of $R$ could be inferred up to sign and scale.  That is, starting with $G$, it is possible to infer $L$, where $G$ is needed to compute $R$.

Now define as $R^*$ the residuals when $L$ is unknown and can't be used in estimation - when we only know $G$.   Now $\exists i, j s.t. R^*_i \not\perp\!\!\!\perp R^*_j | \bar{C}_i, \bar{C}_j$. Intuitively, violation of independence of residuals given graph gives rise to identifiability. 
\end{proof}
\end{theorem}
